...
function editProduct(category, index) {
    ...
    modalContent.innerHTML = buildModalShell({
        ...
        onSave: `saveEditProduct('${category}', ${index})`
...
async function saveEditProduct(category, index) {
...
async function saveNewProduct(category, title) {
    ...
    try {
        ...
        if (data.success) {
            ...
            siteData.products[category].push(data.data);
...
async function deleteProduct(category, index) {
    ...
    try {
        ...
        if (data.success) {
            ...
            // LOCAL ARRAY SE REMOVE KARO
            siteData.products[category].splice(index, 1);
